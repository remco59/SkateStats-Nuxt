...
export function segmentDistances(distanceM: number, nSegments: number): number[] {
  ...
  if (last <= 0) {
    ...
    const even = Math.max(Math.round(remainingDist / remainingSegments), 1)
...
export function computeRaceMetrics(laps: number[], distanceM: number): RaceMetrics {
